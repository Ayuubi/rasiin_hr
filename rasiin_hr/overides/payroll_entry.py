...
class PayrollEntry(Document):
	...
	def get_salary_component_total(
		...
		if salary_components:
			...
			for item in salary_components:
				...
				employee_receivable = self.get_receivable_deduction(component_type, item)
				employee_commission = self.get_commission_earning(component_type, item)
				...
				for cost_center, percentage in employee_cost_centers.items():
					...
					elif employee_receivable:
						self.add_receivable_deduction_entry(
							item, amount_against_cost_center, cost_center, employee_receivable
						)
					elif employee_commission:
						self.add_commission_earning_entry(item, amount_against_cost_center, cost_center, employee_commission)
					else:
	...
	def get_advance_deduction(self, component_type: str, item: dict) -> str | None:
		...
		return
	#receivable
	def get_receivable_deduction(self, component_type: str, item: dict) -> str | None:
		if component_type == "deductions" and item.additional_salary:
			ref_doctype, ref_docname = frappe.db.get_value(
				"Additional Salary",
				item.additional_salary,
			...
			if ref_doctype == "Employee Receivable Deduction":
				return ref_docname
		return

	# Commission
	def get_commission_earning(self, component_type: str, item: dict) -> str | None:
		if component_type == "earnings" and item.additional_salary:
			ref_doctype, ref_docname = frappe.db.get_value(
				"Additional Salary",
				item.additional_salary,
				["ref_doctype", "ref_docname"],
			...
			if ref_doctype == "Employees Commission":
				return ref_docname
		return
	...
	#receivable
	def add_receivable_deduction_entry(
		self,
		item: dict,
		amount: float,
		cost_center: str,
		employee_receivable: str,
	) -> None:
		self._receivable_deduction_entries.append(
			{
				"employee": item.employee,
				"account": self.get_salary_component_account(item.salary_component),
				"amount": amount,
				"cost_center": cost_center,
				"reference_type": "Employee Receivable Deduction",
				"reference_name": employee_receivable,
			}
		)

	# Commission
	def add_commission_earning_entry(self, item: dict, amount: float, cost_center: str, employee_commission: str) -> None:
		self._commission_earning_entries.append(
			{
				"employee": item.employee,
				"account": self.get_salary_component_account(item.salary_component),
				"amount": amount,
...
#Receivable
	def set_accounting_entries_for_receivable_deductions(
		self,
		accounts: list,
		currencies: list,
		company_currency: str,
		...
		#receivable
		for entry in self._receivable_deduction_entries:
			payable_amount = self.get_accounting_entries_and_payable_amount(
				entry.get("account"),
				entry.get("cost_center"),
				entry.get("amount"),
 	...
 	# Commission
	def set_accounting_entries_for_commissions(
		self,
		accounts: list,
		currencies: list,
		company_currency: str,
		...
		# Loop through all the commission entries and create accounting entries for each
		for entry in self._commission_earning_entries:
			payable_amount = self.get_accounting_entries_and_payable_amount(
				entry.get("account"),
				entry.get("cost_center"),
				entry.get("amount"),
	...
	def make_accrual_jv_entry(self):
		...
		self._receivable_deduction_entries = []
		self._commission_earning_entries = []
		...
		if earnings or deductions:
			...
			#receivable
			payable_amount = self.set_accounting_entries_for_receivable_deductions(
				accounts,
				currencies,
				company_currency,
				accounting_dimensions,
			...
			#commission
			payable_amount = self.set_accounting_entries_for_commissions(
				accounts,
				currencies,
				company_currency,
				accounting_dimensions,
